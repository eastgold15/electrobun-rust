...
const rpc = Electroview.defineRPC<TestRunnerRPC>({
  maxRequestTime: 300_000, // 5 minutes for long test runs
  handlers: {
    requests: {},
    messages: {
      ...
      buildConfig: (config) => {
        ...
        console.log(
          `Build config: defaultRenderer=${config.defaultRenderer}, available=[${config.availableRenderers.join(", ")}]`
        );
      },
      updateStatus: (info) => {
        updateUpdateUI(info);
        console.log(
          `Update status: ${info.status}, current=${info.currentVersion}, new=${info.newVersion || "n/a"}`
        );
      },
...
async function init() {
  ...
  btnRunInteractive = document.getElementById(
    "btn-run-interactive"
  ...
  historyToggle = document.getElementById(
  ...
  historyClear = document.getElementById(
...
async function loadTests(retries = 10): Promise<void> {
  ...
  for (let i = 0; i < retries; i++) {
    try {
      if (!electrobun.rpc) {
        console.log(
          `RPC not ready yet, retrying in 500ms (attempt ${i + 1}/${retries})...`
        );
  ...
  testList.innerHTML =
    '<div style="padding: 20px; color: #f87171;">Failed to load tests. Please refresh the window.</div>';
}
...
async function persistSearchQuery(query: string) {
  try {
    await electrobun.rpc?.request.setTestRunnerPreferences({
      searchQuery: query,
    });
  } catch (err) {
...
function updateBuildConfigUI(config: {
  ...
  if (chromiumVersionEl) {
    if (config.cefVersion) {
      ...
      chromiumVersionEl.textContent = chromiumMatch
        ? chromiumMatch[1]!
        : config.cefVersion;
    } else {
...
function addStatusEntryToUI(entry: UpdateStatusEntry) {
  ...
  if (entry.details) {
    ...
    if (
      entry.details.totalPatchesApplied !== undefined &&
      entry.details.totalPatchesApplied > 0
    ) {
