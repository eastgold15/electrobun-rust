...
export class TestExecutor {
  ...
  private interactiveResolver:
    | ((result: { passed: boolean; notes?: string }) => void)
    | null = null;
  private readyResolver: (() => void) | null = null;
  private verificationResolver: ((result: InteractiveResult) => void) | null =
    null;
  ...
  private async cleanupTestWindows(testId: string) {
    ...
    for (const win of windows) {
      try {
        ...
        console.debug(
          `Cleanup: Window ${win.id} already closed or invalid:`,
          (e as Error).message
        );
      }
    }
