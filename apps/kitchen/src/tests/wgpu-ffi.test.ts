...
import { GpuWindow, WGPU, WGPUBridge } from "@pori15/electrobun-rust/bun";
...
function pickSurfaceFormatAlpha(
  ...
  if (formatCount && formatPtr) {
    const formats = new Uint32Array(
      toArrayBuffer(formatPtr, 0, formatCount * 4)
    );
    if (formats.length) {
      format = formats[0]!;
    }
...
`;
      ...
      const renderOnce = () => {
        ...
        if (!texPtr) {
          throw new Error("Surface texture null");
        }
        const textureView = WGPUNative.symbols.wgpuTextureCreateView(texPtr, 0);
        if (!textureView) {
          throw new Error("Texture view null");
        }
