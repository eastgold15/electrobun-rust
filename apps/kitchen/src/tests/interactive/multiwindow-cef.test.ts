...
export const multiwindowCefTests = [
  defineTest({
    ...
    description:
      "Test that 3 CEF windows with OOPIFs all load correctly without mouse movement",
    interactive: true,
    timeout: 120_000,
    async run({ log, showInstructions }) {
      ...
      if (timedOut > 0) {
        ...
        log(
          `WARNING: Windows ${timedOutWindows.join(", ")} timed out - this may indicate the bug is present`
        );
      }
