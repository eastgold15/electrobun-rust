...
export const windowEventTests = [
    defineTest({
        ...
        description:
            "Test both move and resize events are detected with live updates",
        interactive: true,
        timeout: 120_000,
        async run({ log, showInstructions }) {
            ...
            await new Promise<void>((resolve, reject) => {
                ...
                win.on("move", (event: any) => {
                    ...
                    win.webview.rpc?.send.updatePosition({
                        x: Math.round(x),
                        y: Math.round(y),
                    });
                    ...
                    if (!moveDetected) {
                        ...
                        win.webview.rpc?.send.updateStatus({
                            moveDetected,
                            resizeDetected,
                        });
                        checkComplete();
                ...
                win.on("resize", (event: any) => {
                    ...
                    win.webview.rpc?.send.updateSize({
                    ...
                    if (!resizeDetected) {
                        resizeDetected = true;
                        log(
                            `Resize event detected: ${Math.round(width)}x${Math.round(height)}`
                        );
                        win.webview.rpc?.send.updateStatus({
                            moveDetected,
                            resizeDetected,
    ...
    defineTest({
        ...
        category: "Window Events (Interactive)",
        description:
            "Test both blur and focus events are detected with live updates",
        interactive: true,
        timeout: 120_000,
        async run({ log, showInstructions }) {
            await showInstructions([
    ...
    defineTest({
        ...
        async run({
            ...
            if (win.window.isVisibleOnAllWorkspaces()) {
                throw new Error(
                    "Window should not be visible on all workspaces initially"
                );
            }
            ...
            if (!win.window.isVisibleOnAllWorkspaces()) {
                throw new Error(
                    "Window should be visible on all workspaces after setting"
                );
            }

            log(
                "Window is now visible on all workspaces - verify in Mission Control"
            );
            ...
            if (win.window.isVisibleOnAllWorkspaces()) {
                throw new Error(
                    "Window should not be visible on all workspaces after unsetting"
                );
            }
