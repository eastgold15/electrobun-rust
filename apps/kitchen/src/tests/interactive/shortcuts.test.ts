...
export const shortcutTests = [
          ...
          handlers: {
            requests: {
              ...
              unregisterAllShortcuts: () => {
                log(
                  `Unregistering all ${registeredAccelerators.size} shortcuts`
                );
  ...
  defineTest({
    ...
    description:
      "Verify isRegistered correctly reports shortcut registration state",
    interactive: false,
    async run({ log }) {
      ...
      if (!registered) {
        log(
          "WARNING: Could not register any test shortcuts - all candidates in use"
        );
        log(
          "This is common on Linux systems with many global shortcuts already registered"
        );
