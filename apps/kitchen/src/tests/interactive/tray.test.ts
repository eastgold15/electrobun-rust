...
import { BrowserView, BrowserWindow, Tray } from "@pori15/electrobun-rust/bun";
...
export const trayTests = [
  defineTest({
    ...
    description:
      "Interactive playground for testing tray icon, title, and menus",
    interactive: true,
    ...
    async run({ log, showInstructions }) {
      ...
      await new Promise<void>((resolve) => {
        ...
        const rpc = BrowserView.defineRPC<any>({
          maxRequestTime: 600_000, // 10 minutes for interactive exploration
          handlers: {
            requests: {
              closeWindow: () => {
                winRef?.close();
