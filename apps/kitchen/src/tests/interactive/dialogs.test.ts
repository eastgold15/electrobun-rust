...
import { BrowserView, BrowserWindow, Utils } from "@pori15/electrobun-rust/bun";
...
export const dialogTests = [
  ...
  defineTest({
    ...
    async run({ log, showInstructions }) {
      ...
      log(
        `Dialog closed with: ${buttonNames[result.response]} (index ${result.response})`
      );
      log("Test passed - dialog interaction confirmed");
    },
  }),
  ...
  defineTest({
    ...
    category: "Dialogs (Interactive)",
    description:
      "Interactive playground to test file dialog with configurable options",
    interactive: true,
    timeout: 600_000, // 10 minutes for exploration
    async run({ log, showInstructions }) {
      await showInstructions([
      ...
      await new Promise<void>((resolve) => {
        ...
        const rpc = BrowserView.defineRPC<any>({
          maxRequestTime: 600_000, // 10 minutes - file dialogs can take a while
          handlers: {
            requests: {
              closeWindow: () => {
                winRef?.close();
              ...
              openFileDialog: async (opts: any) => {
                ...
                if (
                  opts.startingFolder &&
                  opts.startingFolder.startsWith("~")
                ) {
                  opts.startingFolder = opts.startingFolder.replace(
                    "~",
                    Bun.env["HOME"] || "/Users"
                  );
                }
                ...
                if (result.length > 0 && result[0] !== "") {
                  ...
                  result.forEach((path: string, i: number) =>
                    log(`  ${i + 1}. ${path}`)
                  );
                } else {
