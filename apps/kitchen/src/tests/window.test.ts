...
export const windowTests = [
  ...
  defineTest({
    ...
    category: "BrowserWindow",
    description:
      "Test creating a window without activation and toggling inactive/active show paths",
    async run({ createWindow, log }) {
      log(
        "Focus another app now if you want to observe initial create with activate: false"
      );
      ...
      log(
        "Focus another app now if you want to observe runtime showInactive() behavior"
      );
      await new Promise((resolve) => setTimeout(resolve, 1500));
  ...
  defineTest({
    ...
    async run({ createWindow, log }) {
      ...
      if (finalState) {
        ...
        if (finalState) {
          log(
            "WARNING: Window manager may not properly support minimize/unminimize"
          );
  ...
  defineTest({
    ...
    async run({ createWindow, log }) {
      if (process.platform !== "darwin") {
        log(
          `Skipping macOS-specific fullscreen behavior on ${process.platform}`
        );
        return;
      }
  ...
  defineTest({
    ...
    async run({ createWindow, log }) {
      ...
      if (!isOnTop) {
        ...
        if (!isOnTop) {
          log(
            "WARNING: Window manager may not properly support always-on-top state detection"
          );
  ...
  defineTest({
    ...
    async run({ createWindow, log }) {
      if (process.platform !== "darwin") {
        log(
          `Skipping macOS-specific visibleOnAllWorkspaces behavior on ${process.platform}`
        );
        return;
      }
  ...
  ...(["default", "hiddenInset", "hidden"] satisfies TitleBarStyle[]).map(
    (titleBarStyle) =>
      defineTest({
        name: `Window focus and blur events (titleBarStyle: ${titleBarStyle})`,
        category: "BrowserWindow",
        description:
          "Test that focus and blur events fire when windows gain and lose focus",
        async run({ createWindow, log }) {
  ...
  defineTest({
    ...
    category: "BrowserWindow",
    description:
      "Test creating a window with inset titlebar (transparent titlebar, native controls visible)",
    async run({ createWindow, log }) {
      const win = await createWindow({
        url: "views://test-harness/index.html",
  ...
  defineTest({
    ...
    category: "BrowserWindow",
    description:
      "Test macOS traffic light offset creation and runtime repositioning",
    async run({ createWindow, log }) {
      if (process.platform !== "darwin") {
        log(
          `Skipping macOS-specific traffic light behavior on ${process.platform}`
        );
        return;
      }
  ...
  defineTest({
    ...
    category: "BrowserWindow",
    description:
      "Test programmatically setting window position and size together",
    async run({ createWindow, log }) {
      const win = await createWindow({
        url: "views://test-harness/index.html",
        title: "SetFrame Test",
      ...
      const initialFrame = win.window.getFrame();
      log(
        `Initial frame: (${initialFrame.x}, ${initialFrame.y}) ${initialFrame.width}x${initialFrame.height}`
      );
      ...
      const newFrame = win.window.getFrame();
      log(
        `New frame: (${newFrame.x}, ${newFrame.y}) ${newFrame.width}x${newFrame.height}`
      );
