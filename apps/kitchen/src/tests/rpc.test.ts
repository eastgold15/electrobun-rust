...
import { BrowserView } from "@pori15/electrobun-rust/bun";
...
export const rpcTests = [
  defineTest({
    ...
    category: "RPC",
    description:
      "Test that bun can make RPC requests to webview and receive responses",
  ...
  defineTest({
    ...
    category: "RPC",
    description:
      "Test that webview can make RPC requests to bun and receive responses",
    async run({ createWindow, log }) {
      const rpc = createTestHarnessRPC();
      const win = await createWindow({
        url: "views://test-harness/index.html",
      ...
      const result =
        await win.webview.rpc?.request.evaluateJavascriptWithResponse({
          script:
            "return window.electrobun.rpc.request.add({ a: 100, b: 23 });",
        });
  ...
  defineTest({
    ...
    async run({ createWindow, log }) {
      ...
      const notFound = BrowserView.getById(999_999);
