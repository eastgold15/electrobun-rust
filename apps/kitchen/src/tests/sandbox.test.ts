...
export const sandboxTests = [
  ...
  defineTest({
    ...
    category: "Sandbox",
    description:
      "Test that dom-ready and navigation events still fire in sandbox mode",
    timeout: 15_000,
    async run({ createWindow, log }) {
  ...
  defineTest({
    ...
    category: "Sandbox",
    description:
      "Verify events work in normal (non-sandboxed) mode for comparison",
    timeout: 15_000,
    async run({ createWindow, log }) {
      let domReadyFired = false;
  ...
  defineTest({
    ...
    category: "Sandbox",
    description:
      "Test that webview tags (OOPIFs) are blocked in sandboxed windows",
    timeout: 20_000,
    async run({ createWindow, log }) {
  ...
  defineTest({
    ...
    category: "Sandbox",
    description:
      "Test that webview tags (OOPIFs) load in non-sandboxed windows",
    timeout: 20_000,
    async run({ createWindow, log }) {
      // Get initial BrowserView count
      const viewsBefore = BrowserView.getAll().length;
