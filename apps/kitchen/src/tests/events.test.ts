...
import Electrobun, { Utils } from "@pori15/electrobun-rust/bun";
...
export const eventsTests = [
  ...
  defineTest({
    ...
    category: "Events",
    description:
      "Test that window-specific events don't fire for other windows",
    async run({ createWindow, log }) {
      let win1Events = 0;
      let win2Events = 0;
  ...
  defineTest({
    ...
    category: "Events",
    description:
      "Test that before-quit event fires and can cancel application quit",
    async run({ log }) {
  ...
  defineTest({
    ...
    category: "Events",
    description:
      "Test that per-window close handlers fire before global close handlers",
    async run({ createWindow, log }) {
      const order: string[] = [];
