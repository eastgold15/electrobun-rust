...
const testRunnerRPC = BrowserView.defineRPC<TestRunnerRPC>({
  ...
  handlers: {
    requests: {
      ...
      runAllAutomated: async () => await executor.runAllAutomated(),
      runInteractiveTests: async () => await executor.runInteractiveTests(),
      ...
      getUpdateStatusHistory: () => Updater.getStatusHistory(),
      clearUpdateStatusHistory: () => {
        Updater.clearStatusHistory();
      },
      getTestRunnerPreferences: () => testRunnerPreferences,
