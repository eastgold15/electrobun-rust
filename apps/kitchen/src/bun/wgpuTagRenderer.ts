...
const WGPUVertexFormat_Float32x4 = 0x00_00_00_1f;
...
function createPipeline(
  ...
  for (let i = 0; i < attributes.length; i += 1) {
    ...
    const attrDesc = makeVertexAttribute(
      attr.offset,
      attr.location,
      attr.format
    );
...
export class WgpuTagRenderer {
  ...
  start(viewId: number, win: BrowserWindow<any>, rect: Rect) {
    ...
    const view =
      WGPUView.getById(viewId) ??
      WGPUView.adoptExisting(viewId, {
        windowId: win.id,
        autoResize: false,
        frame: rect,
      });
