...
fn fix_cef_symlinks(app_dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !cef_dir.exists() {
        info!("CEF directory not found, skipping symlink creation");
        return Ok(());
    }
    ...
    for lib in cef_libs {
        ...
        #[cfg(unix)]
        {
            ...
            if let Err(e) = symlink(&target_path, &symlink_path) {
                info!("Warning: Could not create symlink for {}: {}", lib, e);
            } else {
                info!("Created symlink: {} -> {}", lib, target_path);
            }
        }
    }
...
fn remove_quarantine(app_dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
    info!("Removing quarantine attributes from: {:?}", app_dir);
    ...
    if output.status.success() {
        ...
        info!("Warning: xattr returned non-zero exit code");
    }
...
fn create_desktop_shortcut(
    ...
    if !desktop_dir.exists() {
        info!("Note: Desktop directory not found; skipping Desktop shortcut creation");
        return Ok(());
    }

    let launcher_path = app_dir.join("bin").join("launcher");
    if !launcher_path.exists() {
        info!("Warning: launcher binary not found at {:?}", launcher_path);
        return Ok(());
    }
            ...
            if let Some(ext) = path.extension() {
                if ext == "desktop" {
                    ...
                    if let Ok(mut file) = File::create(&desktop_path) {
                        ...
                        info!("Created desktop shortcut: {:?}", desktop_path);
                    }
    ...
    if !found_desktop_file {
        info!("Warning: No desktop file found in extracted app directory");
    }
...
fn create_windows_shortcut(
    ...
    if !target_path.exists() {
        info!("Warning: Could not find launcher.exe at {:?}", target_path);
        return Ok(());
    }
    ...
    info!("Created Windows shortcuts for: {}", metadata.name);
...
"#,
    ...
    if output.status.success() {
        info!("Created Windows shortcut: {:?}", lnk_path);
    } else {
        info!("Warning: Could not create shortcut");
    }
...
"#,
    ...
    if let Ok(mut file) = File::create(&reg_path) {
        ...
        info!("Created uninstall registry file: {:?}", reg_path);
    }
...
fn replace_self_with_launcher(
    ...
    if !launcher_path.exists() {
        info!("Warning: Could not find launcher at {:?}", launcher_path);
        return Ok(());
    }

    fs::copy(&launcher_path, exe_path)?;
    info!("Replaced self with launcher shortcut from: {:?}", launcher_path);
...
fn extract_tar(tar_data: &[u8], extract_dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
    info!("DEBUG: Starting tar extraction to: {:?}", extract_dir);
    info!("DEBUG: Tar data size: {} bytes", tar_data.len());
...
fn extract_and_install(
    ...
    #[cfg(any(target_os = "linux", target_os = "windows"))]
    {
        ...
        info!("✓ Saved tar file ({} bytes)", decompressed_data.len());
    }
...
fn extract_from_self() -> Result<bool, Box<dyn std::error::Error>> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if final_metadata_path.exists() && final_archive_path.exists() {
            info!("Found adjacent archive file: {:?}", final_archive_path);
            ...
            info!("Using metadata: identifier={}, name={}, channel={}",
            ...
            info!("Extracting to: {:?}", self_extraction_dir);
    ...
    info!("Self-extracting archive found at offset {}", archive_offset);
    info!("Extracting to: {:?}", self_extraction_dir);
...
fn main() {
    ...
    info!("Electrobun self-extractor v1.3 starting...");
    ...
    #[cfg(any(target_os = "windows", target_os = "linux"))]
    {
        match extract_from_self() {
            Ok(true) => {}
            Ok(false) => {
                error!("ERROR: Not a valid self-extracting installer");
                std::process::exit(1);
            }
            Err(e) => {
                error!("ERROR: {}", e);
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        info!("macOS extraction not fully implemented in this version");
    }
