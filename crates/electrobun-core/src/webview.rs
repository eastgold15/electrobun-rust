...
use crate::{WEBVIEW_REGISTRY, window};
...
/// Create a new webview — 发送命令到事件循环线程用 wry 真正创建
pub fn create_webview(options: WebviewOptions) -> Result<u32, ElectrobunError> {
    ...
    let preload_script = WEBVIEW_RUNTIME_STATE.lock().unwrap_or_else(|e| e.into_inner())
        .preload_script
        .clone()
        .unwrap_or_default();
    ...
    let state = WebviewState {
        ...
        preload_script: preload_script.clone(),
        devtools_open: false,
        zoom: 1.0,
        can_go_back: false,
    ...
    let mut registry = WEBVIEW_REGISTRY.lock().unwrap_or_else(|e| e.into_inner());
    registry.insert(id, state);
    drop(registry);

    // CEF webviews don't use wry — only register state, skip wry creation
    if !options.use_wry {
        return Ok(id);
    }

    // 发送 CreateWebview 命令到事件循环线程，让 wry 真正创建
    let (result_tx, result_rx) = mpsc::channel();
    let cmd = WindowCommand::CreateWebview {
        id,
        window_id: options.window_id,
        url: options.url,
        x: options.bounds.x,
    ...
    if !window::send_command(cmd, Duration::from_secs(5)) {
        // 事件循环尚未启动，命令在 channel 中排队，乐观返回
        return Ok(id);
    }
...
/// Close a webview — 发送命令到事件循环线程销毁真正的 wry WebView
pub fn close_webview(id: u32) -> bool {
    let (result_tx, result_rx) = mpsc::channel();
    let cmd = WindowCommand::CloseWebview { id, result_tx };
    if !window::send_command(cmd, Duration::from_secs(1)) {
        // 事件循环未启动，只清理 registry
        let mut registry = WEBVIEW_REGISTRY.lock().unwrap_or_else(|e| e.into_inner());
        return registry.remove(&id).is_some();
    }
    let result = window::wait_bool_result(&result_rx, Duration::from_secs(1));
    // registry 清理由 handle_command 的 CloseWebview 分支完成
    result
}

/// Navigate webview to URL — 通过命令通道在事件循环线程上执行
pub fn webview_navigate(id: u32, url: &str) -> bool {
    ...
    let (result_tx, result_rx) = mpsc::channel();
    let cmd = WindowCommand::NavigateWebview {
        id,
        url: url.to_string(),
        result_tx,
    };
    ...
    window::wait_bool_result(&result_rx, Duration::from_secs(1))
}

/// Set webview bounds — 通过命令通道
pub fn set_webview_bounds(id: u32, x: f64, y: f64, width: f64, height: f64) -> bool {
    ...
    let (result_tx, result_rx) = mpsc::channel();
    let cmd = WindowCommand::SetWebviewBounds { id, x, y, width, height, result_tx };
    if !window::send_command(cmd, Duration::from_secs(1)) {
        return true;
    }
    window::wait_bool_result(&result_rx, Duration::from_secs(1))
...
pub fn get_webview_bounds(id: u32) -> Option<Rect> {
    let registry = WEBVIEW_REGISTRY.lock().unwrap_or_else(|e| e.into_inner());
...
/// Load HTML content directly into the webview — 通过命令通道
pub fn load_html(id: u32, html: &str) -> bool {
    ...
    let (result_tx, result_rx) = mpsc::channel();
    let cmd = WindowCommand::LoadHTMLWebview {
        id,
        html: html.to_string(),
        result_tx,
    };
...
/// Evaluate JavaScript in the webview — 通过命令通道
pub fn evaluate_javascript(id: u32, js: &str) -> bool {
    ...
    let (result_tx, result_rx) = mpsc::channel();
    let cmd = WindowCommand::EvaluateJSWebview {
        id,
        js: js.to_string(),
        result_tx,
    };
