...
fn is_dev_build(exe_dir: &str) -> bool {
    ...
    let version_path = PathBuf::from(exe_dir)
        .join("..")
        .join("Resources")
        .join("version.json");
    // fs::read_to_string opens, reads, and closes in one call
    //      如果失败返回 Err，用 .ok()? 转为 None
    let Ok(content) = fs::read_to_string(&version_path) else {
        return false;
    };
    // Parsing with serde_json
    let Ok(parsed) = serde_json::from_str::<HashMap<String, serde_json::Value>>(&content) else {
        return false;
    };
...
extern "C" fn signal_handler(sig: i32) {
    if sig == SIGINT {
        ...
        let mut count = SIGINT_COUNT.lock().expect("value should exist");
        *count += 1;
    ...
    let pid = *CHILD_PID.lock().expect("value should exist");
    unsafe {
        kill(pid, sig);
    }

    if sig == SIGTERM {
        *SHOULD_EXIT.lock().expect("value should exist") = true;
    }
}
...
fn main() {
    ...
    info!("Launcher starting on {}...", std::env::consts::OS);
    info!("Current directory: {}", exe_dir_str);
    ...
    let (program, args): (String, Vec<String>) = match main_process {
        "bun" => {
            ...
            let resources_path = PathBuf::from(exe_dir)
                ...
                .join("main.js");
            (
                bun_path.to_str().expect("value should exist").to_string(),
                vec![resources_path.to_str().expect("value should exist").to_string()],
            )
        },
        _ => {
            error!("未知的主进程类型: {}", main_process);
            std::process::exit(1);
        },
    };
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        info!("Setting LD_PRELOAD: {}", preload_libs.join(":"));
    }
    ...
    if force_console {
        info!("Console mode forced via ELECTROBUN_CONSOLE=1");
    } else if is_dev_build {
        info!("Dev build detected - console output enabled");
    }
    ...
    if !is_dev_build {
        ...
        let mut cwd_w: Vec<u16> = exe_dir_str
            .encode_utf16()
        ...
        info!("Child process spawned with PID {}", pi.dwProcessId);
    ...
    *CHILD_PID.lock().expect("value should exist") = child.id();
    ...
    match result.code() {
        ...
        None => {
            // 被信号杀死（Unix）
            #[cfg(unix)]
            info!("Child process terminated by signal");
            std::process::exit(1);
        },
    }
}
