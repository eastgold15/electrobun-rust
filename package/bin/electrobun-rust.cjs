...
const cacheDir = join(electrobunDir, '.electrobun-cache-rust');
const cliBinary = join(cacheDir, `electrobun${binExt}`);

// GitHub repo configuration
const GITHUB_OWNER = 'eastgold15';
const GITHUB_REPO = 'electrobun-rust';
...
async function ensureCliBinary() {
  ...
  console.log('Downloading electrobun-rust CLI for your platform...');
  ...
  // TODO: Update to your own GitHub repo
  const tarballUrl = `https://github.com/${GITHUB_OWNER}/${GITHUB_REPO}/releases/download/${tag}/electrobun-cli-${platform}-${arch}.tar.gz`;
  ...
  try {
    ...
    console.log('electrobun-rust CLI downloaded successfully!');
    return binLocation;

  } catch (error) {
    throw new Error(`Failed to download electrobun-rust CLI: ${error.message}`);
  }
}
...
async function main() {
  try {
    ...
    child.on('error', (error) => {
      console.error('Failed to start electrobun-rust CLI:', error.message);
      process.exit(1);
    });
