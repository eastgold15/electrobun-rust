...
pub fn open_asar(path: &Path) -> Result<AsarArchive, String> {
    ...
    let header_size = u32::from_le_bytes(size_buf[..4].try_into().unwrap()) as u64;

    // 如果最高位为 1，实际用 8 字节
    let (real_header_size, pad) = if header_size & 0x8000_0000 != 0 {
        let _high = (header_size & 0x7FFF_FFFF) as u64;
...
fn resolve_path<'a>(
    ...
    for (i, part) in parts.iter().enumerate() {
        let node = current.get(*part).ok_or_else(|| {
            format!("路径 '{}' 在 '{}' 处未找到", path, part)
        })?;
...
fn assign_offsets(files: &mut HashMap<String, AsarNode>, base: &mut u64) {
    ...
    for name in names {
        let node = files.get_mut(&name).unwrap();
...
fn ensure_path<'a>(
    ...
    unsafe {
        for (i, part) in parts.iter().enumerate() {
            ...
            if i < parts.len() - 1 {
                let node = map.get_mut(*part).unwrap();
                if node.files.is_none() {
                    node.files = Some(HashMap::new());
                }
                current = node.files.as_mut().unwrap();
            }
        }
        // 返回最后一个父节点所在的 map
        if parts.len() <= 1 {
        ...
        for p in last_parent {
            cur = cur.get_mut(*p).unwrap().files.as_mut().unwrap();
        }
        cur
    }
}
...
pub fn pack_directory(
    ...
    // 写入文件数据段
    for (_asar_path, fs_path) in &file_entries {
...
pub unsafe extern "C" fn asar_open(path: *const c_char) -> *mut std::ffi::c_void {
    ...
    let archive = match open_asar(Path::new(c_str)) {
        Ok(a) => a,
        Err(_) => return std::ptr::null_mut(),
    };
...
pub unsafe extern "C" fn asar_read_file(
    ...
    let data = match read_file(archive, c_str) {
        Ok(d) => d,
        Err(_) => return -1,
    };
