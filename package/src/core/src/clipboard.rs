...
pub fn read_text() -> Result<String, ElectrobunError> {
    let _lock = CLIPBOARD_MUTEX.lock().map_err(|e| ElectrobunError::OperationFailed(e.to_string()))?;
...
pub fn read_image_base64() -> Result<String, ElectrobunError> {
    ...
    Err(ElectrobunError::OperationFailed("Image clipboard not yet implemented".to_string()))
}
