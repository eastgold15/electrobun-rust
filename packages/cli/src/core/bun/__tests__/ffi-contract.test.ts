...
(isUnix ? describe : describe.skip)(
  ...
  () => {
    it("dlopen + FFIType.cstring + FFIType.u64 (strlen)", () => {
      ...
      const len = lib.symbols.strlen(new TextEncoder().encode("hello\0"));
    ...
    it("ptr + toArrayBuffer round-trip", () => {
      ...
      const back = new Uint8Array(toArrayBuffer(ptr(src), 0, src.byteLength));
    ...
    it("JSCallback: native invokes JS via function pointer (qsort)", () => {
      ...
      lib.symbols.qsort(ptr(arr), BigInt(arr.length), 4n, compare.ptr);
