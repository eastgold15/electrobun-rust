...
export class BrowserView<T extends RPCWithTransport = RPCWithTransport> {
  ...
  init() {
    ...
    return ffi.request.createWebview({
      ...
      hostWebviewId: this.hostWebviewId ?? undefined,
  ...
  loadHTML(html: string) {
    ...
    // Load HTML content directly into webview
  ...
  stopFindInPage() {
    ffi.request.webviewStopFind({ id: this.id, stopAll: true });
  }
      ...
      send(message: any) {
        ...
        if (!sentOverSocket) {
          try {
            ...
            console.error(
              "host: failed to serialize message to webview",
              error
            );
          }
        }
