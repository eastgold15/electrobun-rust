...
import { ffi, native } from "../proc";

export const moveToTrash = (path: string) => ffi.request.moveToTrash({ path });

export const showItemInFolder = (path: string) =>
  ffi.request.showItemInFolder({ path });
...
export const openPath = (path: string): boolean =>
  ffi.request.openPath({ path });

export const setDockIconVisible = (visible: boolean): void => {
  ffi.request.setDockIconVisible({ visible });
};

export const isDockIconVisible = (): boolean => ffi.request.isDockIconVisible();
...
export const clipboardReadImage = (): Uint8Array | null =>
  ffi.request.clipboardReadImage();
...
export const clipboardAvailableFormats = (): string[] =>
  ffi.request.clipboardAvailableFormats();
