...
const Updater = {
  ...
  downloadUpdate: async () => {
    ...
    const latestHash = (await Updater.checkForUpdate()).hash;
    ...
    if (updateInfo?.updateReady) {
      ...
      if (await Bun.file(latestTarPath).exists()) {
        ...
        if (currentOS === "linux") {
          ...
          const bundleStats = statSync(newAppBundlePath, {
            throwIfNoEntry: false,
          });
          if (!(bundleStats && bundleStats.isDirectory())) {
            console.error(
              `App bundle directory not found at: ${newAppBundlePath}`
            );
...
`;
  ...
  localInfo: {
    version: async () => (await Updater.getLocalInfo()).version,
    hash: async () => (await Updater.getLocalInfo()).hash,
    channel: async () => (await Updater.getLocalInfo()).channel,
    baseUrl: async () => (await Updater.getLocalInfo()).baseUrl,
  },
