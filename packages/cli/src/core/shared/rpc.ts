...
export type RPCOptions<S extends RPCSchema, RS extends RPCSchema> = Pick<
  _RPCAllOptions<S>,
...
>(options: _RPCAllOptions<Schema> = {}) {
  ...
  function setRequestHandler(h: RPCRequestHandler<Schema["requests"]>) {
    ...
    requestHandler = (method, params) => {
      ...
      if (handlerFn) {
        return (handlerFn as any)(params);
      }
  ...
  function requestFn<M extends keyof RemoteSchema["requests"]>(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (maxRequestTime !== Number.POSITIVE_INFINITY) {
        requestTimeouts.set(
          requestId,
          setTimeout(() => {
  ...
  const sendProxy = send as unknown as RPCMessagesProxy<Schema["messages"]>;
  ...
  function addMessageListener<M extends keyof RemoteSchema["messages"]>(
    message: M,
  ...
  function removeMessageListener<M extends keyof RemoteSchema["messages"]>(
    message: M,
    listener: RPCMessageHandlerFn<RemoteSchema["messages"], M>
  ): void;
  ...
  async function handler(message: _RPCPacket) {
    ...
    if (message.type === "message") {
      ...
      for (const listener of listeners) {
        (listener as any)(message.payload);
      }
      return;
    }
